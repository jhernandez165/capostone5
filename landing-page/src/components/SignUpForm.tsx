...
import {
  ...
  SignUpFormProgress,
  SignUpFormStep,
} from "@src/modules/SignUpFormComponents";
...
> {
  ...
  async onSubmit(
    ...
    helpers: FormikHelpers<SignUpFormSchema>,
  ) {
    const createApplicant: CreateApplicant = {
      ...
      mailingAddress: values.sameAsBilling
        ? values.address
      ...
      mailingZipcode: values.sameAsBilling
        ? values.zipcode
    ...
    try {
      const { data, status } =
        await this.apiService.postApplication(applyRequest);
    ...
    return (
      <div>
        {!this.state.submitted ? (
          ...
          >
            {({ errors, touched, dirty, values, isValid, isSubmitting }) => (
              <Form>
                ...
                <Prompt
                  when={dirty}
                  message="You've already started signing up. Are you sure you want to leave?"
                />
                ...
                {this.state.errorMessage ? (
                  <div
                    key={this.state.errorMessage.replace(/\\s/g, "")}
          ...
          <SignUpResults data={this.state.data!} />
        )}
      </div>
    );
  }
